package com.t4cloud.boot.support.controller;
...
import com.t4cloud.boot.core.base.utils.JwtUtil;
import com.t4cloud.boot.core.base.utils.RedisUtil;
import com.t4cloud.boot.support.entity.SupResource;
import com.t4cloud.boot.support.service.ISupResourceService;
...
@RequestMapping("/T4Cloud-Support/file")
public class FileController extends T4Controller<SupResource, ISupResourceService> {
...
//            rocketMQTemplate.convertAndSend(TOPIC_RESOURCE, resource);
    ...
    @ApiOperation(position = 3, value = "视频预览(未完成）", notes = "直接拼接相对文件路径即可，此接口header中的token如果有的话需要传递过来，如果访问的是私有资源的元，需要校验token的")
    public void video(
            @ApiParam("上传资源所得的资源路径")
            @PathVariable String path,
