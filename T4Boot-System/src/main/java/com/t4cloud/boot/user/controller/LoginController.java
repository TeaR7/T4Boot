package com.t4cloud.boot.user.controller;
...
import com.t4cloud.boot.core.base.constant.CacheConstant;
import com.t4cloud.boot.core.base.constant.LogConstant;
import com.t4cloud.boot.core.base.controller.T4Controller;
import com.t4cloud.boot.core.base.entity.LoginUser;
import com.t4cloud.boot.core.base.entity.dto.R;
import com.t4cloud.boot.core.base.utils.PwdUtil;
import com.t4cloud.boot.core.base.utils.RedisUtil;
import com.t4cloud.boot.core.base.utils.UUIDUtil;
import com.t4cloud.boot.core.base.utils.UserUtil;
import com.t4cloud.boot.support.entity.SupMessage;
import com.t4cloud.boot.support.service.ISupMessageService;
import com.t4cloud.boot.system.entity.SysRole;
import com.t4cloud.boot.system.entity.SysUserRole;
import com.t4cloud.boot.system.service.ISysRoleService;
...
import com.t4cloud.boot.user.entity.dto.LoginDTO;
...
import static com.t4cloud.boot.core.base.constant.CacheConstant.SYS_USERS_BIND_CODE;
import static com.t4cloud.boot.core.base.constant.CacheConstant.SYS_USERS_CHECK_CODE;
...
@RestController
@AllArgsConstructor
@RequestMapping("/T4Cloud-User/login")
...
public class LoginController extends T4Controller<SysUser, ISysUserService> {
    ...
    @Autowired
    private ISupMessageService supMessageService;
    ...
    public R<JSONObject> getCheckCode() {
        ...
        try {
            ...
            String key = DigestUtil.md5Hex(code + System.currentTimeMillis());
    ...
    public R<LoginUser> loginByCode(
            @ApiParam("第三方类型，微信：WE_OPEN，其他暂不支持") @PathVariable("type") String type,
        ...
        SysRole sysRole = sysRoleService.appUserRole(role);
        ...
        switch (type) {
            case "WE_OPEN":
                ...
                SysUserThird userThird = weUserService.userJsOauth(code, wechatConfig);

                //根据loginType和unionId查询第三表
                SysUserThird sysUserThird = sysUserThirdService.lambdaQuery().eq(SysUserThird::getLoginType, userThird.getLoginType())
                        .eq(SysUserThird::getOutId, userThird.getOutId()).one();

                //如果为null的话则是新用户，先插入
                if (null == sysUserThird) {
                    //如果没查到就开始注册用户
                    sysUserThird = new SysUserThird();
                    BeanUtil.copyProperties(userThird, sysUserThird);

                    //然后插入用户表
                    user = new SysUser();
                    //配置用户信息
                    String username = EmojiParser.removeAllEmojis(sysUserThird.getNickName());
//                    String username = sysUserThird.getNickName();
                    ...
                    userRole.setRoleIds(sysRole.getId());
...
//                    service.updateById(user);
    ...
    public R<JSONObject> getCaptcha(
        ...
        //判断手机还是邮箱
        SupMessage supMessage = new SupMessage();
        supMessage.setMessageTemplateCode(target.contains("@") ? "EMAIL-CAPTCHA" : "PHONE-CAPTCHA");
        supMessage.setTarget(target);
        //设置参数
        JSONObject param = new JSONObject();
        param.set("username", target);
        param.set("code", captcha);
        supMessage.setParam(param.toString());

        supMessageService.saveByTemplate(supMessage);
    ...
    public R<LoginUser> register(@Valid @RequestBody LoginDTO loginDTO, BindingResult bindingResult) {
        ...
        SysRole sysRole = sysRoleService.appUserRole(roleCode);
        ...
        userRole.setUserId(user.getId());
        userRole.setRoleIds(sysRole.getId());
        sysUserRoleService.saveByUser(userRole);
